...
class StudentController extends CI_Controller {
        ...
        foreach($college as $row_college):
            if (($row_college->college_id != "11") && ($row_college->college_id != "12")) {
                ...
                foreach($course as $row_course):
                    if (!in_array($row_course->course_id, array(0, 144, 145))) 
                    {
        ...
        if ($result !== true) {
            ...
            if ($status == 4) {
                $payment_mail = "<br><p style='margin: 0; '>Pay your fees thru the following payment options:</p>
                                    ...
                                    <p style='margin: 0; '>2. Landbank Cash Deposit</p>
                                    ...
                                    <br>
                                    <p style='margin: 0; '><b>NOTE: </b>We strictly accept OFFICIAL RECEIPT from the CLSU Cashier Only.</p>
                                    ...
                                    Link: <a href='https://tinyurl.com/2p8dfdpj'>https://tinyurl.com/2p8dfdpj</a></p>
                                    <br>
            ...
            try {
                ...
                $mail->Host       = 'smtp.gmail.com';
                $mail->SMTPAuth   = true;
                $mail->Username   = 'clsuoad.noreply@clsu2.edu.ph';
                $mail->Password   = 'AD315510N5';
                $mail->SMTPSecure = 'tls';
                $mail->Port       = 587;
                ...
                $mail->ClearReplyTos();
                $mail->addAddress($email, $fullname);
                $mail->setFrom('clsuoad.noreply@clsu2.edu.ph', 'OAD - DRMS');
                ...
                /*$subject = "Request confirmed [Request ID: ".$uniq_request_id."]";
                $body = "<p style='margin: 0; '>Good day, ".ucwords($firstname)."!</p>
                ...
                $send_email = $this->sendEmail($email, $subject, $body, $fullname);
                $data = array (
                    'status'        =>  1,
                    'title'         =>  'Request was successfully sent. <br>Please screenshot or take note of your REQUEST ID for further concerns.',
                    'request_id'    =>  $uniq_request_id,
                    'emailStat'     =>  $send_email
                );
    ...
    public function insert_inactive_request() {
        ...
        if ($result !== true) {
            ...
            // $staff_id = 0;
            if (isset($staff)) {
                if($staff->staff_id_frontline != '') {
                    $staff_id = "'".$staff->staff_id_frontline."'"  ;
                    $staff_name = ucwords($staff->staff_fname.' '.$staff->staff_lname);
    ...
    public function sendEmail($emailTo, $txtSubject, $body, $name)
    {
        // Multiple recipients
        // $to = 'johny@example.com, sally@example.com'; // note the comma
        $to = $emailTo; // note the comma
        ...
        $headers[] = 'From: OAD - DRMS <clsuoad.noreply22@clsu2.edu.ph>';
